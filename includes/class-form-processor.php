...
class Asaas_Form_Processor {
    ...
    private function process_single_donation($api, $sanitized_data) {
        ...
        $payment_data = [
            'customer' => $sanitized_data['customer_id'],
            'value' => $sanitized_data['donation_value'],
            'dueDate' => date('Y-m-d'), // Data atual para vencimento
            'description' => 'Doação única'
        ];
        
        // Definir o tipo de cobrança com base no método de pagamento
        if (!empty($sanitized_data['payment_method'])) {
            switch ($sanitized_data['payment_method']) {
                case 'card':
                    $payment_data['billingType'] = 'CREDIT_CARD';
                    // Para cartão de crédito, adicionar o token
                    if (!empty($sanitized_data['card_token'])) {
                        $payment_data['creditCardToken'] = $sanitized_data['card_token'];
                    } else {
                        return [
                            'success' => false,
                            'errors' => ['Token do cartão de crédito não disponível']
                        ];
                    }
                    break;
                case 'pix':
                ...
                case 'boleto':
                    ...
                    $payment_data['canBePaidAfterDueDate'] = true; // Permite pagamento após vencimento
                    break;
                default:
                    return [
                        'success' => false,
